...
| `.env` | Laravel and Compose: **`APP_URL`**, **`APP_HOST`**, database, Redis, optional **`PROXY_NETWORK`** / **`BACKEND_NETWORK`**. |
...
- **`DB_*`** — from `provision-db` (e.g. `DB_HOST=postgres`, `DB_CONNECTION=pgsql`, …). The image includes **PDO MySQL** and **PDO PostgreSQL**; match **`DB_CONNECTION`** to what you provision.
- **`REDIS_*`** — `REDIS_HOST=redis`, password from the host stack `/opt/stack/.env` if applicable
- `CACHE_STORE=redis`, `SESSION_DRIVER=redis`, `QUEUE_CONNECTION=redis` (recommended with **`dms-worker`**)
...
| Database / Redis | **`backend`** network, credentials in **`.env`**. |
| Queues not processing | **`dms-worker`** running (`docker compose ps`), **`QUEUE_CONNECTION=redis`**, Redis reachable. |
